...
export class AdvancedDamageCalculatorRoute
  extends LitElement
  implements BeforeEnterObserver
{
  ...
  render(): TemplateResult {
    ...
    if(this.selectedWeapon && this.targetUnit) {
      canTarget = DamageCalculator.canWeaponTargetUnit(this.selectedWeapon, this.targetUnit);
    }
    ...
    const weaponStatsTable = [
      ...
      },
      {
        ...
        value: this.calculatorOutput?.damagePerShot.toFixed(2),
      },
      {
      ...
      },
      {
        ...
        value: this.calculatorOutput?.suppressionDamagePerShot.toFixed(2),
      },
      {
        label: 'Accuracy',
    ...
    if (this.selectedWeapon?.missileProperties) {
      weaponStatsTable.push(
        ...
        },
        {
          ...
          value: `${this.calculatorOutput?.missileTravelTimeToTarget?.toFixed(
            2
          )} s`,
        }
      );
    ...
    const optimalTable = [
      ...
      },
      {
        ...
        value: this.calculatorOutput?.shotsToSuppress.toFixed(1),
      },
      {
        label: 'Time to Suppress',
        value: `${Math.max(
          this.calculatorOutput?.timeToSuppress,
          0
        ).toFixed(2)} s`,
      },
    ];
    ...
    const averageTable = [
      ...
      },
      {
        label: 'Shots to Suppress',
        value:
          this.calculatorOutput?.shotsToSuppressWithAccuracy.toFixed(1),
      },

      {
        label: 'Time to Suppress',
        value: `${Math.max(
          this.calculatorOutput?.averageTimeToSuppress,
          0
        ).toFixed(2)} s`,
      },
    ];
