...
export class PackArmouryCard extends LitElement {
  ...
  armouryCardSelected(veterancy: number) {
    if (this.pack && this.deck) {
      ...
      const availableWithoutTransports =
        this.deck.getPackAvailableWithoutTransport(this.pack);

      // If there is only one transport, add the unit with the transport
      if (
        transports &&
        transports.length === 1 &&
        !availableWithoutTransports
      ) {
        this.deck.addUnit({
          veterancy,
          pack: this.pack,
      ...
      // If there are multiple transports, open the dialog
  ...
  transportSelected(transport: Unit) {
    if (this.pack && this.selectedUnitVeterancy !== null) {
      this.deck?.addUnit({
        transport,
        veterancy: this.selectedUnitVeterancy,
        pack: this.pack,
      });
    }
  ...
  render(): TemplateResult {
    if (this.deck && this.pack) {
      const disabled =
        this.deck.getAvailableQuantityOfPack(this.pack) > 0 ? false : true;
      ...
      return html` <transport-selection
        ...
        <armoury-card
          .pack=${this.pack}
          .deck=${this.deck}
          @add-button-clicked=${(event: CustomEvent) =>
            this.armouryCardSelected(event.detail.veterancy)}
          .disabled=${disabled}
        ></armoury-card>`;
    }
